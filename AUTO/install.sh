...
# cd /tmp && yum install wget -y && wget https://github.com/munishgaurav5/ks/raw/master/AUTO/install.sh && chmod 777 install.sh && ./install.sh 
...
menuentry "reinstall" {
    $root_value
    linux /vmlinuz ip=${IPADDR}::${GW}:${PREFIX}:$(hostname):$Boot_device:off nameserver=$DNS1 nameserver=$DNS2  inst.vnc inst.vncconnect=${IPADDR}:1 inst.vncpassword=changeme  inst.repo=$MIRROR inst.ks=hd:${boot_hd}:/${KSFName} inst.lang=en_US inst.keymap=us 
    initrd /initrd.img
}
EOF
     else
...
menuentry "reinstall" {
    $root_value
    linux /boot/vmlinuz  ip=${IPADDR}::${GW}:${PREFIX}:$(hostname):$Boot_device:off nameserver=$DNS1 nameserver=$DNS2  inst.vnc inst.vncconnect=${IPADDR}:1 inst.vncpassword=changeme  inst.repo=$MIRROR inst.ks=hd:${boot_hd}:/boot/${KSFName} inst.lang=en_US inst.keymap=us 
