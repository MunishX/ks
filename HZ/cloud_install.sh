...
menuentry "reinstall" {
    $root_value
    linux /vmlinuz ip=${IPADDR} nameserver=$DNS1 nameserver=$DNS2  inst.repo=$MIRROR inst.ks=hd:${boot_hd}:/${KSFName} inst.lang=en_US inst.keymap=us 
    initrd /initrd.img
}
EOF
     else
...
menuentry "reinstall" {
    $root_value
    linux /boot/vmlinuz ip=${IPADDR} nameserver=$DNS1 nameserver=$DNS2  inst.repo=$MIRROR inst.ks=hd:${boot_hd}:/boot/${KSFName} inst.lang=en_US inst.keymap=us 
