...
menuentry "reinstall" {
    set root=(hd0,1)
    linux /vmlinuz inst.vnc inst.vncpassword=changeme inst.headless net.ifnames=0 biosdevname=0 ip=${IPADDR}::${GW}:${PREFIX}:$(hostname):eth0:off nameserver=$DNS1 nameserver=$DNS2 inst.repo=$MIRROR inst.ks=$KSURL
    initrd /initrd.img
}
EOF
